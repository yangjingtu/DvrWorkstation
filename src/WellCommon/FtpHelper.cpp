...
UINT CFtpHelper::PutFile()
{
	while( ::WaitForSingleObject(m_hEvent, INFINITE) == WAIT_OBJECT_0 )
	{
		...
		if(m_wstrPutFile.empty() || m_strSaveFile.empty())
			continue;
		
		//第一个参数为本地文件的地址，第二个为服务器端文件的地址
		if(m_pFtpConnection->PutFile(m_wstrPutFile.c_str(), m_strSaveFile.c_str()))
		{
			m_wstrPutFile.clear();
			m_strPutFile.clear();
			m_strSaveFile.clear();
...
UINT CFtpHelper::_PutFile(const wstring& strFileName, const wstring& saveFile)
{
	...
	m_wstrPutFile = strFileName;
	m_strPutFile = WS2S(m_wstrPutFile);
	SetEvent(m_hEvent);	
	...
	try
	{
		...
		{ 
			...
			{ 
				...
				if(m_regCB.tfCB != NULL && TRANS_SKIP == (nRlt = m_regCB.tfCB(WS2S(strTmp.GetString()))) )
				{
					continue;
				}
				...
				m_wstrPutFile = tempFind.GetFileName(); 
				m_strPutFile = WS2S(m_wstrPutFile);
				int len = 0;
...
void CFtpHelper::GetTransSpeed(string& strspd)
{ 
	strspd = m_strSpeed;
}

void CFtpHelper::GetPutFile(string& strpf)
{
	strpf = m_strPutFile;
}
...
void CALLBACK CFtpHelper::PutTimerProc(HWND hwnd,UINT uMsg,UINT_PTR idEvent,DWORD dwTime)
{
	...
	if(pFtp->m_llTransSize > 100)
	{
		...
		pFtp->m_wstrSpeed = strSd.GetString();
		pFtp->m_strSpeed = WS2S(pFtp->m_wstrSpeed);
	}
}
...
{
	return _CreateDirctory(strPath.c_str());
}
...
{
	return _PutFile(strFile.c_str(), saveFile.c_str());
}
...
{
	return _TransFolder(strFoldername.c_str(), strDst.c_str());
}
