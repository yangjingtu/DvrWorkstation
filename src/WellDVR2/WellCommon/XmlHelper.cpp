...
bool CXmlHelper::Load(const string& file, bool bCreate)
{
	try
	{
		...
		if(len == 0)
		{
			if(bCreate)
			{
				bool bLoad = m_doc.LoadFile(file.c_str(), TIXML_ENCODING_UTF8);
...
TiXmlElement* CXmlHelper::GetNode(const string& strPath)
{
	...
	try
	{
		vector<string> ret = CStringHelper::split(strPath, "\\");
...
{	
	string strVal = GetNodeValue(WS2S(strPath));
	return S2WS(strVal);
}
...
string CXmlHelper::GetNodeValue(const string& strPath)
{
	...
	const char* chText = pNode->GetText();
	return chText == NULL ? "" : chText;
}
