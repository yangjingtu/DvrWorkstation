...
vector<string> g_vecUsb;
map<string, char> g_mapUsb;

DWORD GetRegKeyValue(LPCSTR strSubKey, LPCSTR strKey, DWORD keyType, LPCSTR strValue, DWORD dwLen);
...
)
{
	...
	LPCSTR strSubKeyPrefix = "SYSTEM\\CurrentControlSet\\Enum\\";
	LPCSTR strSKey1 = "SYSTEM\\MountedDevices";
	TCHAR strSubKey[200] = {0};
	LPCSTR strKey = "ParentIdPrefix";//要查询的键名称
	TCHAR strValue[512] = {0};
	TCHAR strValue1[26][256] = {0};
    ...
    for (index=1; index <= NumPorts; index++)
    {
		...
		//if (connectionInfo->ConnectionStatus != NoDeviceConnected)
        {
			...
			if(deviceDesc)
			{
				string str(deviceDesc);
				if(str.find("USB\\VID_0595&PID_2002\\") != string::npos)
				{
					//////////////////////////////////////////////////////////////////////////
					//add by yjt 2014-06-25
					memset(strSubKey, 0, sizeof(strSubKey));
					strcat(strSubKey, strSubKeyPrefix);
					strcat(strSubKey, deviceDesc);
					dwLen = sizeof(strValue);
					...
					if( (dwLen = GetRegKeyValue(strSubKey, strKey, REG_SZ, strValue, dwLen)) != 0)
					{
						string strP(strValue);
						printf("%s", strValue);
						for (n = 0; n < 26; ++n)
						{
							str = strValue1[n];
							if(str.find(strP) != string::npos)
							{
								g_vecUsb.push_back(str);
								diskName = 'A' + n;
								break;
							}
						}
					...
					else
					{
						...
						//取出ZORAN01234567
						string strId = str.substr( str.rfind('\\') + 1);
						//查找#ZORAN01234567, 和有ParentIdPrefix区别开
						strId = "#" + strId;
						for (n = 0; n < 26; ++n)
						{
							str = strValue1[n];
							if(str.find(strId) != string::npos)
							{
								g_vecUsb.push_back(str);
								diskName = 'A' + n;
								break;
           ...
           (PUCHAR)commonDesc + commonDesc->bLength <= descEnd)
    {
        switch (commonDesc->bDescriptorType)
        {
            case USB_CONFIGURATION_DESCRIPTOR_TYPE:
                ...
                //(PUCHAR)commonDesc += (PUCHAR)commonDesc->bLength;
...
DWORD GetRegKeyValue(LPCSTR strSubKey, LPCSTR strKey, DWORD keyType, LPCSTR strValue, DWORD dwLen)
{
	...
	long ret0 = RegOpenKeyEx(hKeyRoot,strSubKey,0,KEY_READ,&hKEY);
	...
	// hKEY是上面打开时得到的指针。
	ret0 = RegQueryValueEx(hKEY,strKey,NULL,&keyType, (LPBYTE)strValue, &dwLen);
